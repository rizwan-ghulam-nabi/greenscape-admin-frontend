// app/admin/dashboard/orders/processing/page.js
'use client';
...
export default function ProcessingOrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_BASE_URL}/orders?status=processing`, {
...
const saveChanges = async () => {
  try {
    ...
    // ✅ 1. Update order status (using order route)
    if (orderStatus !== selectedOrder.orderStatus) {
      ...
      if (res.data?.emailSent) {
        emailWasSent = true;
        setEmailSent(true);
        setEmailNotification(`📧 Email sent to ${selectedOrder.user?.email}`);
      }
    }
    
    // ✅ 2. Update payment status (using payment route)
    if (paymentStatus !== selectedOrder.paymentStatus) {
      // First find the payment for this order
      const paymentsRes = await axios.get(`${API_BASE_URL}/payments`, {
        withCredentials: true
      });
      
      // Find payment that belongs to this order
      const payment = paymentsRes.data.payments?.find(
        p => p.order === selectedOrder._id || p.order?._id === selectedOrder._id
      );
      
      if (payment) {
        // ✅ Use payment route
        await axios.put(
          `${API_BASE_URL}/payments/${payment._id}/status`,
          { status: paymentStatus },
          { withCredentials: true }
        );
        console.log('✅ Payment status updated via /payments/:id/status');
      } else {
    ...
    alert(`✅ Status updated successfully!${emailWasSent ? ' 📧 Email sent to customer.' : ''}`);
    setShowModal(false);
    fetchOrders();
    
  } catch (err) {
    console.error('❌ Error:', err);
    const errorMessage = err.response?.data?.error || err.message || 'Failed to update';
    alert(`❌ ${errorMessage}`);
  } finally {
    setSaving(false);
  }
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-gray-600 font-medium">No processing orders found</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${getOrderStatusBadge(order.orderStatus)}`}>
                        {getOrderStatusIcon(order.orderStatus)} {order.orderStatus || 'Processing'}
                      </span>
                    </td>
