// app/admin/dashboard/orders/cancelled/page.js
'use client';
...
export default function CancelledOrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_BASE_URL}/orders?status=cancelled`, {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage orders that have been cancelled
          </p>
        </div>
        <button
          onClick={fetchOrders}
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-gray-600 font-medium">No cancelled orders found</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${getOrderStatusBadge(order.orderStatus)}`}>
                        {getOrderStatusIcon(order.orderStatus)} {order.orderStatus || 'Cancelled'}
                      </span>
                    </td>
